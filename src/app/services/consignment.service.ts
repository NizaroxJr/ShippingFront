...
export class ConsignmentService {
  serverAPI_URL = `https://shippingapi-production.up.railway.app`;
  ...
  deliverConsignment(payload: {
    ...
    const user_roles = [];
    [...payload.employee.roles].forEach((role) =>
    ...
    const httpOptions = this.httpService.getHttpHeader();
    return this.http.post<{ user: User; consignment: Consignment }>(
      url,
      body,
      httpOptions
    );
